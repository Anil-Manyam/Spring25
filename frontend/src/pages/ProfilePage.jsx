...
export default function ProfilePage() {
  ...
  const handleSubmitPayment = async () => {
    if (!payAmount || !cardNumber)         return alert("Please fill all fields.");
    if (!/^[0-9]{12}$/.test(cardNumber))   return alert("Card number must be exactly 12 digits.");
    if (parseFloat(payAmount) > fine)      return alert(`You cannot pay more than $${fine}.`);
      ...
      {(!Array.isArray(history) || history.length === 0) ? (
        ...
        history.map(h => (
          ...
          >
            <strong>{h.title}</strong> (ISBN: {h.isbn})<br />
            Borrowed:&nbsp;{new Date(h.borrow_date).toLocaleDateString()}<br />
            Returned:&nbsp;{new Date(h.return_date).toLocaleDateString()}
          </div>
        ))
      )}

      {/* ---------- librarian-only user-management console ---------- */}
      {user.user_type === "librarian" && <UserManagement />}

      {/* Navigation buttons */}
      <button style={{ marginRight: "10px" }} onClick={() => navigate("/home")}>
        Back to Home
      </button>
