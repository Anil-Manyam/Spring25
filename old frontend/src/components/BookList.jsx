...
//   const [feedbacks, setFeedbacks] = useState([]);  // moved inside functional component
...
//         if (data.fine > 0) {
//           msg += ` Fine due: $${data.fine}`;
//         }
//         alert(msg);

//         // prompt for optional feedback
//         const feedback = window.prompt("Would you like to leave feedback for this book? (optional)");
...
//   const handleShowFeedback = async (bookId) => {
//     try {
//       const res = await fetch(`http://127.0.0.1:5000/api/books/${bookId}/feedback`);
//       const data = await res.json();
//       if (data.error) {
//         alert(data.error);
...
//       } else {
//         const reviews = data.map(f => `- ${f.username}: ${f.feedback}`).join("\n\n");
//         alert(`Reviews:\n\n${reviews}`);
//       }
//     } catch {
//       alert("Server error fetching reviews.");
//     }
//   };
...
//       ) : (
//         filteredBooks.map((b) => (
//           <div className="book-card" key={b.book_id} style={{
//             display: "flex",
...
//             <div>
//               {b.available ? (
//                 <BorrowButton user={user} bookId={b.book_id} onBorrow={() => window.location.reload()} />
//               ) : (
...
//               )}
//               <button
//                 onClick={() => handleShowFeedback(b.book_id)}
